...
class PMSTestCase(TestCase):
    ...
    def test_listar_roles(self):
        ...
        '''
        User.objects.create_user('john2', 'lennon@thebeatles.com', 'johnpassword')
        c = Client()
        c.login(username='john2', password='johnpassword')
        resp = c.get('/roles/')
        self.assertEqual(resp.status_code, 200)
    ...
    def test_detalle(self):
        ...
        self.assertEqual(resp.context['rol'].name, 'Rol 1')
